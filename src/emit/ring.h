...
namespace emit
{
    ...
    public:
        ...
        // `bytes` rounded down to a power of two. False, and inactive, when the budget is too small
        // or the allocation fails, so the caller falls back to the synchronous path.
        ...
        // Consumer, single thread only. `out` must hold the biggest record ever deposited. False
        // when nothing is ready: empty, or the next record's producer has not committed yet.
