...
function Clear-XRayStaleTraces([int]$ExcelPid) {
    ...
    #
    # No live process can share our pid, so every same-pid file here is a dead
    # process's -- safe to delete, and parallel-safe by construction. Under
    # -SessionMode Reuse an earlier test in THIS process may own one, but it has
    # already read and asserted its own trace by the time the next test sets up.
    if ($ExcelPid -le 0) { return }
    $dir = Join-Path (Get-XRayRoot) 'TraceFiles'
    Get-ChildItem (Join-Path $dir ("XRayXL_Trace_*_{0}.csv" -f $ExcelPid)) -ErrorAction SilentlyContinue |
