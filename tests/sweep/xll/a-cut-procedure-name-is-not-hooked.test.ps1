...
try {
    ...
    # TxRegisterLongName picks a fresh Excel name every call and returns which one, so a
    # reused session cannot find the name already registered and skip the whole point.
    ...
    $which = [int]$app.Run('TxRegisterLongName')
    $longName = "TxLongNamed$which"
    ...
    $registered = [string]$app.Evaluate("$longName(1)")
    ...
    $misnamed = @($rows | Where-Object { $_.kind -eq 'exit' -and $_.function -eq $longName -and $_.ret -ne '1.75' })
    $caller = @($rows | Where-Object { $_.kind -eq 'entry' -and $_.function -eq 'TxCallsPrefix' })

    Check 'excel-registered-the-long-name' ($registered -eq '1.75') "$longName(1)='$registered'"
    ...
    Check 'no-row-carries-the-cut-name' ($misnamed.Count -eq 0) `
          "$longName exits not returning the long export's 1.75: $($misnamed.Count)"
