...
| `vba\` | Identity, the shadow stack, nesting, recursion, error unwinds and frame layouts. Files under `cases\` state a VBA workload and the arithmetic the totals must satisfy (statements, procedures, depth, frames opened == closed, per-frame `outcome`); bespoke tests take the ones that need a class module, a form, two workbooks, a spilled array or a second arming session, including `co-compiled` (every case's setup in one module) and `return-value-from-cell` (every Function's result at every depth — scalars, Variants, five array shapes, Subs and Property Lets that must report NO value — against the values Excel computed). `labels\` is its own session with `XRAYXL_DIAG`, so each signature names the opcode that typed it: a parameter only handed to a Variant is typed by the Variant's label, every declared type ByVal and ByRef, and an array parameter only ReDimmed by the `ReDim`; a parameter its own code never types by its caller's call -- literals, a chain of passed-on parameters -- while a local passed by address, a cell, `Application.Run` and an expression argument stay untyped; by the procedure it is passed to, compiled first, and not before an uncompiled callee has run; and `calls-built-from-expressions-...`, 42 procedures called with nested calls, expressions before and after, named and omitted arguments, Property Let, Variant and record results, record members, array elements and `With`, each parameter's type and value stated or stated untyped |
...
| `ribbon\` | The one part of the add-in that is a COM object, and the one allowed to fail. The add-in connects AND nothing of its transient registration outlives the connect -- which is only interesting together, since no keys and no add-in is a ribbon that never loaded; no Excel add-ins entry at `LoadBehavior=3`, which used to autoload the tracer into the next Excel; state driven from `Application.Run` reaches the ribbon with no COM entry point faulting and no setting lost; and, under `off\`, that `XRAYXL_RIBBON=0` costs the ribbon buttons and nothing else. **These sessions are headless, so Excel never builds the ribbon** -- `onLoad` fires only where there is a window -- and each test says which of the two runs it was rather than passing for a call it never made |
...
| `unit\` | The product's own code, with no Excel: one folder per test holding a C++ program built by `XRayXL.sln` from the shipping sources, and the `.test.ps1` that runs it and reports each check as a case — the caller decoder, the registration-string parser, the trace-parameter grammar, the value decoder, the procedure table, the p-code scan guard and slot roles, the walk's last-statement check, its Variant label, and the caller's call site, the output ring under contention, the row formatter, closing a session under writers, and the ribbon's decisions -- enablement, what each control reads and writes, and a cross-check of the shipped customUI XML against the handlers in both directions, which is what catches a dead control; and the Options dialog itself, opened outside Excel and driven through its pages, the Format drop-down, the embedded licence and notices, Apply (greyed until something differs, and what it logs), Cancel and the armed lock; and the Diagnostics dialog, driven the same way -- its three tables, the search, the column sort, multiple selection, and a teardown check that opens and closes it twenty times and asserts its window class is unregistered every time and that no GDI or USER object is left behind. **These two are the only tests of either dialog**: neither is reachable from a sweep session, which is headless, so Excel never builds the ribbon that opens them |
