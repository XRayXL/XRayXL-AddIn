...
'@
    ...
    $txbEntries = @(Read-TraceFile (Get-XRayTraceCsv $sx.ProcId) | Where-Object { $_.kind -eq 'entry' -and $_.function -eq 'TxB' }).Count
    Check 'xll-count-matches-the-trace' ($finalTxb -and ([double]$finalTxb.Calls -eq $txbEntries)) `
          "summary TxB calls=$(if ($finalTxb) { $finalTxb.Calls } else { 'not listed' }), TxB entry rows=$txbEntries"
