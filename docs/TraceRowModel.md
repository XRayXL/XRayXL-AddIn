...
| `breaks` | uint32 | **Optional — only in a file traced with `BREAKPOINTS` on.** On every VBA exit row, how many times that activation stopped at a breakpoint in the VBA editor; `0` when it did not. A non-zero count means `ticks` includes time spent in the debugger, not running. Its callers' `ticks` include that time too, though their own count is `0`. Empty on every other row |
